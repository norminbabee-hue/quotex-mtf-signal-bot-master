...
class MultiPairScanner:
    ...
    def __init__(self, adapter: MarketDataAdapter, on_signal: Callable[[Signal], None], *, server_offset_seconds: int | float = 6 * 60 * 60, candidates: Iterable[str] | None = None) -> None:
    ...
    def refresh(self) -> ScannerSnapshot:
        ...
        self.managers = {symbol: LiveCandleManager(self.adapter, self._broker_symbols[symbol], server_offset_seconds=self.server_offset_seconds) for symbol in symbols}
    ...
    def preview_candidates(self, now_utc: datetime, lead_seconds: int = 45) -> list[Signal]:
        """Evaluate all pairs and return actionable candidates in the pre-entry window."""
    ...
    def on_tick(self, tick: Tick) -> None:
        ...
        if broker_symbol is None or service is None or manager is None or tick.symbol != broker_symbol:
            return
        events = manager.on_tick(tick)
        events.sort(key=lambda event: self._TIMEFRAME_PRIORITY[event.candle.timeframe_seconds])
        for event in events:
            for signal in service.on_closed_candle(event.candle):
