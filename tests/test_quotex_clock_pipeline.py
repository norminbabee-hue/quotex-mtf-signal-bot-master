...
def tick(seconds: int, price: str) -> Tick:
    return Tick(
        symbol="EURUSD",
        timestamp_utc=datetime.fromtimestamp(seconds, tz=timezone.utc),
        bid=Decimal(price),
        ask=Decimal(price),
    )
...
def test_m1_boundary_uses_quotex_server_clock():
    ...
    assert builder.update(tick(16 * 3600 + 59 * 60 + 59, "1.1000")) is None
    closed = builder.update(tick(17 * 3600, "1.1002"))
...
def test_m5_boundary_uses_quotex_server_clock():
    ...
    builder.update(tick(16 * 3600 + 54 * 60 + 59, "1.1000"))
    closed = builder.update(tick(16 * 3600 + 55 * 60, "1.1003"))
